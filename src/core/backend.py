...
def open_window(params,host,port):
    import webview
...
------------------------------------------------------------------------
        ...
        window_thread=Process(target=open_window, daemon=True,args=(params,startUp.config['api-ip'],startUp.config['api-port']))
        window_thread.start()
        socketio.run(app=app,debug=False, host=startUp.config['api-ip'], port=startUp.config['api-port'],allow_unsafe_werkzeug=True)
    except Exception as e:
        queue.put({'text':traceback.format_exc(),'level':'error'})
        time.sleep(3)
    finally:

        window_thread.terminate()
        if listener_thread:
